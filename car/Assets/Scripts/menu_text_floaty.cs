...
public class menu_text_floaty : MonoBehaviour {
    ...
    private void textFloat_function()
    {
        if (textFloat)
        {
            ...
            //lerp to size
            text_object.GetComponent<TextMeshPro>().fontSize = Mathf.Lerp(text_object.GetComponent<TextMeshPro>().fontSize, fontSize_original, font_lerp_speed);
        }
        else
        {
            text_object.GetComponent<TextMeshPro>().fontSize = Mathf.Lerp(text_object.GetComponent<TextMeshPro>().fontSize, fontSize_small, font_lerp_speed);
        }
    }
